...
form_body =u"""
<form methnod="POST" action="/cgi-bin/dbpole.py">
    好きな軽量言語は？<br />
        %s
        <input type="submit" />
    </form>"""
...
def incrementvalue(cur, lang_name):
    ...
    for item in cur.fetchall():
        cur.execute("""UPDATE language_pole SET value=%d WHERE name='%s'""" % (item[0]+1, lang_name))

    if not item:
        cur.execute("""INSERT INTO language_pole(name, value) VALUES('%s', 1)""" % lang_name)
